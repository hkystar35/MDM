...
jamf_jq="install-jq"
zscaler_service="zpa" # zpa, zia, zdx
...
status1="$(echo $zscaler_Status | jq -r ".${zscaler_service}.authenticationStatus")" # = AUTHENTICATED
status2="$(echo $zscaler_Status | jq -r ".${zscaler_service}.serviceStatus")" # = TUNNEL_FORWARDING

/bin/echo "<result>$(echo $status1 $status2 | tr ' ' ';')</result>"
